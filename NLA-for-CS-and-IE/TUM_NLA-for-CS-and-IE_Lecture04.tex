...
\begin{enumerate}
\item Write a function called \mintinline{julia}{RowMajorForwardSubstitution} that implements the forward substitution as described in slide \#3.
\item Write a function called \mintinline{julia}{ColumnMajorForwardSubstitution} that implements forward substitution in way that the components of the lower triangular matrix are fetched in a column-wise fashion.
...
\item Write a function called \mintinline{julia}{get_LU} that returns the $L$ and $U$ of a matrix $A$ factors obtained by forward elimination without pivoting.
...
\item Write a function called \mintinline{julia}{ColumnMajor_LU_InPlace!} that resorts to a column-wise data access pattern for in-place LU factorization without pivoting.
