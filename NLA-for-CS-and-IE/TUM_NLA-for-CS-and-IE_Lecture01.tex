...
\item[-] A matrix $\mathbf{A}\in\mathbb{F}^{m\times n}$ is \textbf{row-column-rank} if $\dim(\mathrm{range}(\mathbf{A}^H))=m$.\vspace{-.15cm}
...
\begin{enumerate}
\item[1.] \textbf{Consistency}: A linear system is consistent if $\mathbf{b}\in\mathrm{range}(A)$, which means there exists at least one vector $\mathbf{x}\in\mathbb{F}^n$ such that $\mathbf{A}\mathbf{x}=\mathbf{b}$.\tinyskip\\
