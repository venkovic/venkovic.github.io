...
% Slide 01
\begin{frame}{Krylov subspace methods for few eigen-pairs}
\begin{itemize}
\item So far we've seen:
\begin{itemize}\normalsize
\item[-] \textbf{Power iterations}, \textbf{inverse iterations} and \textbf{Rayleigh quotient iterations} to compute a single eigen-pair
\item[-] \textbf{QR iterations}, the \textbf{divide-and-conquer method} and the \textbf{method of bissection} to compute all the eigen-pairs of a small-to-medium size and dense matrix
\item[-] \textbf{LOBPCG} to compute a few extremal generalized eigen-pairs of a large, possibly sparse matrix pencil $(A,B)$.
\end{itemize}
\item \textbf{Krylov subspace methods} are another set of iterative methods to compute a few eigen-pairs of a large matrix $A$
\begin{itemize}\normalsize
...
\begin{itemize}
\item Given a vector $v\in\mathbb{R}^n$, the $m$-th Krylov subspace of $A\in\mathbb{R}^{n\times n}$ is 
\begin{align*}
\mathcal{K}_m(A,v)=\mathrm{span}\{v,Av,\dots,A^{m-1}v\}.
\end{align*}
\item The Arnoldi process which we present in this section is a procedure to generate an orthogonal $Q_m:=[q_1,\dots,q_m]$, i.e., $Q_m^TQ_m=I_m$ such that 
\begin{align*}
\mathrm{span}\{q_1,\dots,q_m\}=\mathcal{K}_m(A,v).
\end{align*}
The orthonormal basis in the columns of $Q_m$ is such that $Q_m^TAQ_m=H_m$ is an upper Hessenberg matrix. 
\item We present two different ways to derive Arnoldi procedures:
\begin{enumerate}\normalsize
\item Deduction of Arnoldi iteration from the $AQ=QH$ relation.\vspace{.1cm}
\item Orthogonalization of $Aq_k$ against $q_1,\dots,q_k$.
\end{enumerate}
\item Later, we see that approximate eigen-pairs of $A$ can be sought for within the the Kylov subspace $\mathcal{K}_m(A,v)$.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item We recall from lecture 7 that Householder transformations can be used to transform a matrix into Hessenberg form:
\begin{align*}
Q^TAQ=H
\end{align*}
where $Q^TQ=QQ^T=I_n$.
\item The eigenvalues of $H$ are the same as those of $A$, which can be exploited to find eigenvalues of $A$.
\item Here, instead of considering the full Hessenberg matrix $H$, we approximate eigen-pairs of $A$ with the eigen-pairs of a leading $k$-by-$k$ block $H_k$ of $H$.\vspace{.1cm}
\begin{center}
\includegraphics[height=2.5cm]{images/hessenberg-reduction.png}
\end{center}
\item As it turns out, the eigen-pairs of the leading $k$-by-$k$ block of $H$ are good approximations of some eigen-pairs of $A$.
\end{itemize}
\end{frame}
...
\begin{itemize}
\item Computing the leading $k$-by-$k$ block of $H$ is called the \textbf{Arnoldi process}.
We will see it is similar to the Gram-Schmidt procedure.
...
\begin{align*}
\mathrm{Arnoldi}:
(X_{:,1},m)\in\mathbb{R}^{n}\times\mathbb{N}
\mapsto
Q=[q_1,\dots,q_m]\in\mathbb{R}^{n\times m}
\end{align*}
s.t. $Q^TQ=I_m$ and $\mathrm{Span}\{q_1,\dots,q_m\}=\mathrm{Span}\{q_1,Aq_1,\dots,A^{m-1}q_1\}$ where $q_1:=X_{:,1}/\|X_{:,1}\|_2$.\vspace{.1cm}
\item We are interested by the QR decomposition $X=QR$ such that $X_{:,j}:=Aq_{j-1}$ for $j=2,\dots,m$.
$X$ is defined column-by-column w.r.t. $Q$, so that the Gram-Schmidt procedure is particularly well adapted.\vspace{.1cm}
\item Let $\Pi^{(j)}$ be a projector onto $\mathrm{Span}\{q_1,\dots,q_j\}^\perp$, then Arnoldi$(X_{:,1},m)$ is given by the following GS procedure:
\begin{algorithm}[H]
\small
\caption{$\mathrm{Arnoldi}:(X_{:,1},m)\mapsto Q$}
\begin{algorithmic}[1]
...
Given that $X_{:,j}=Aq_{j-1}$ for $j=2,\dots,m$, we have $R_{ij}=Q_{:,i}^TX_{:,j}=q_i^TAq_{j-1}$ for $(i,j)\in[1,m]\times[2,m]$.
...
\vspace{.1cm}
\item So as to explicitly state the dimension of $Q$ during intermediate states $j<m$ of the Arnoldi algorithm, we write $Q_j:=[q_1,\dots,q_j]$.
...
\begin{algorithm}[H]
\small
\caption{CGS-based Arnoldi$:(X_{:,1},m)\mapsto Q_m$}
\begin{algorithmic}[1]
...
\STATE{$H_{1:j-1,j-1}:=Q_{j-1}^TX_{:,j}$}
\STATE{$q_{j}:=X_{:,j}-Q_{j-1}H_{1:j-1,j-1}$}
\STATE{$h_{j,j-1}:=\|q_{j}\|_2$}\COMMENT{$H_{j+1:m,j-1}:=0$}
...
To distinguish from the relation $AQ=QH$ obtained only when $m=n$.
...
\frametitle{CGS-based Arnoldi procedure}	
\begin{itemize}
\item Approximate solutions in $\mathcal{K}_m(A,r_0)$ for linear systems and eigenvalue problems have residuals which depend on the product $AQ_m$.
\item Exploit the Arnoldi relation $AQ_m=Q_{m+1}\underline{H}_m$ for faster computation.
\item The Arnoldi algorithm is reformulated as follows so as to compute $\underline{H}_m$ at the $m$-th iteration:
\begin{algorithm}[H]
\small
...
\STATE{$H_{j+1,j}:=\|q_{j+1}\|_2$}\COMMENT{$H_{j+2:m+1,j}:=0$}
...
\begin{algorithm}[H]
\small
\caption{MGS-based Arnoldi$:(X_{:,1},m)\mapsto Q_m$}
\begin{algorithmic}[1]
\STATE{$q_{1}:=X_{:,1}/\|X_{:,1}\|_2$}
\FOR{$j=2,\dots,m$}
\STATE{$q_{j}:=Aq_{j-1}$}
...
\end{algorithmic}
\end{algorithm}		
\item For all $(i,j)\in[1,j-1]\times[2,m]$, prior to executing line 5, we have
\begin{align*}
...
\frametitle{MGS-based Arnoldi procedure}	
\begin{itemize}
\item Similarly as before, we want the upper Hessenberg matrix $\underline{H}_m$ to be computed at the end of the $m$-th iteration.
...
\begin{frame}{Arnoldi procedure with Rayleigh-Ritz vectors}
	...
	\item Rayleigh-Ritz projections are \textbf{commonly defined with respect to Krylov subspaces} whose bases $Q_m$ are obtained by a Arnoldi procedure:
	\begin{itemize}
	\item Then, a \textbf{Rayleigh-Ritz vector} $y\in\mathcal{R}(Q_m)$ approximates an eigenvector of $A$ with the \textbf{Ritz value} $\lambda$ such that $Ay-\lambda y\perp \mathcal{R}(Q_m)$. That is, we \textbf{search for} $(\lambda,\hat{y})\in\mathbb{C}\times\mathbb{C}^{m}\setminus\{0\}$ s.t. $z^H\left(Ay-\lambda y\right)=0\;\forall\; z\in\mathcal{R}(Q_m)$ with $\!y=Q_m\hat{y}$.
	This \textbf{simplifies to}
	\begin{align*}
		Q_m^H\left(AQ_m\hat{y}-\lambda Q_m\hat{y}\right)=&\;0\\
		H_m\hat{y}-\lambda\hat{y}=&\;0
		\;\;\implies\;\;
		\boxed{H_m\hat{y}=\lambda\hat{y}}
	\end{align*}
	where use was made of the Arnoldi relation and $Q_m^HQ_m=I_n$.
	\item $k\!<\!m$ $\hspace{-.05cm}$\textbf{dominant $\hspace{-.05cm}$eigenpairs} $\hspace{-.05cm}$$\{(\lambda_\ell,\hat{y}_\ell)\}_{\ell=1}^k$ $\hspace{-.05cm}$\textbf{of} $\hspace{-.05cm}$$H_m$ $\hspace{-.05cm}$are $\hspace{-.05cm}$\textbf{used $\hspace{-.05cm}$to $\hspace{-.05cm}$approximate}$\hspace{-1cm}$\\
	the \textbf{dominant eigenpairs of} $A$ \textbf{with} $\{(\lambda_\ell,y_\ell)\}_{\ell=1}^k$ where $y_\ell:=Q_m\hat{y}_\ell$.
	\end{itemize}
	\end{itemize}
	\medskip
...
\begin{frame}{Arnoldi procedure with Rayleigh-Ritz vectors, cont'd\textsubscript{1}}
	...
	\begin{align*}
		y_\ell^HAy_\ell=(Q_m\hat{y}_\ell)^HAQ_m\hat{y}_\ell=\hat{y}^H_\ell Q_m^HAQ_m\hat{y}_\ell=
		\hat{y}_\ell^HH_m\hat{y}_\ell=\lambda_\ell\hat{y}_\ell^H\hat{y}_\ell=\lambda_\ell
	\end{align*}
	...
	\begin{align*}
		\tilde{r}_\ell=&\;AQ_m\hat{y}_\ell-\lambda_\ell Q_m\hat{y}_\ell\\
		=&\;Q_{m}H_m\hat{y}_\ell+h_{m+1,m}q_{m+1}e_m^T\hat{y}_\ell-\lambda_\ell Q_m\hat{y}_\ell\\
		=&\;\lambda_\ell Q_m\hat{y}_\ell+h_{m+1,m}q_{m+1}e_m^T\hat{y}_\ell-\lambda_\ell Q_m\hat{y}_\ell\\
		=&\;h_{m+1,m}q_{m+1}e_m^T\hat{y}_\ell
	\end{align*}
	\vspace{-1.1cm}\\
	\begin{align*}
		\hspace{.55cm}\boxed{\tilde{r}_\ell=\beta_{m,\ell}q_{m+1}}
		\hspace{.5cm}\mathrm{where}\;\;\boxed{\beta_{m,\ell}:=h_{m+1,m}e_m^T\hat{y}_\ell}.
	\end{align*}
	\begin{itemize}
	\item Essentially, \textbf{the eigen-residuals} $\tilde{r}_1,\dots,\tilde{r}_m$ \textbf{of the Rayleigh-Ritz vectors} $y_1,\dots,y_m$ defined with respect to the \textbf{Krylov subspace} $\mathcal{K}_m(A,q_1)$ are \textbf{all parallel}, \textbf{along the Arnoldi vector} $q_{m+1}$.
	\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Arnoldi procedure with Rayleigh-Ritz vectors, cont'd\textsubscript{2}}
	\begin{itemize}
	\item  From the fact that $\tilde{r}_\ell=\beta_{m,\ell}q_{m+1}$, the norm of the eigen-residual is such that $\|\tilde{r}_\ell\|_2^2=|\beta_{m,\ell}|^2q_{m+1}^Hq_{m+1}=|\beta_{m,\ell}|^2$ where $|\beta_{m,\ell}|=|h_{m+1,m}|\,|e_m^T\hat{y}_\ell|$.
	\begin{itemize}
	\item Consequently, \textbf{a stopping criterion of the form} $\|\tilde{r}_\ell\|_2<\epsilon|\lambda_\ell|$ \textbf{can be checked efficiently} at every iteration \textbf{without} having to compute the \textbf{matrix-vector product} $Ay_\ell$ or even to assemble the vector $y_\ell:=Q_m\hat{y}_\ell$.
	\end{itemize}
	\item As explained earlier,
	\begin{itemize}
	\item the \textbf{orthogonalization} which is at the root of the Arnoldi procedure has time complexity $\mathcal{O}(m^2n)$,
	\item the \textbf{reduced eigensolve} of $H_m$ has time complexity $\mathcal{O}(m^3)$,
	\item the \textbf{storage of the Arnoldi basis} in  $Q_m$ has space complexity $\mathcal{O}(mn)$ so that, \textbf{if convergence is not achieved} for some number $m$ of iterations, it is \textbf{necessary to start the Arnoldi procedure over} with a new initial vector $q_1$.
	\end{itemize}
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs}
	...
	\begin{enumerate}
	\item[1.] Factorizing the shifted operator $A-\sigma I_n$ is not always possible.
	...
	\begin{itemize}
	\item Consider the case in which we are equipped with a basis for the search space $\mathcal{R}(P)$ stored in the columns of $P$.\vspace{.025cm}
	\item Let $Q:=(A-\sigma I_n)P$, and consider the Rayleigh-Ritz pairs of the shift-and-invert operator $(A-\sigma I_n)^{-1}$ with respect to $\mathcal{R}(Q)$.
	\end{itemize}
	\end{itemize}
	\medskip
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs, cont'd\textsubscript{1}}
	...
	\item[]
	\begin{itemize} 
	\item[] That is, consider the pair $(\vartheta,Q\hat{y})$ such that
	\begin{align*}
		Q^H(A-\sigma I_n)^{-1}Q\hat{y}=&\;\vartheta Q^HQ\hat{y},
	\end{align*}
	which develops into the reduced generalized eigenvalue problem
	\begin{align*}
		P^H(A-\sigma I_n)^HP\hat{y}=&\;\vartheta P^H(A-\sigma I_n)^H(A-\sigma I_n)P\hat{y}
	\end{align*}
	which does not require any application of the shift-and-invert operator.
	\item Resulting from a Rayleigh-Ritz projection of the shift-and-invert operator $(A-\sigma I_n)^{-1}$, the pair $(\sigma+1/\vartheta,Q\hat{y})$ should be a good approximation with respect to $\mathcal{R}(Q)$ of the eigenpair closest to $\sigma$.
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs, cont'd\textsubscript{2}}
	...
	\item[]
	\begin{itemize}
	\item Consequently, Morgan (1991) proposes a Petrov-Galerkin projection and seeks for pairs $(\sigma+\lambda,y)$ to approximate eigenpairs of $A$ near $\sigma$ with respect to $\mathcal{R}(P)$, leading to the following procedure:
	\begin{align*}
		\text{Find $\lambda$ and }
		y\in\mathcal{R}(P)
		\text{ s.t. }
		(A-\sigma I_n)y-\lambda y\perp(A-\sigma I_n)\mathcal{R}(P),
	\end{align*}
	...
	\begin{align*}
	P^H(A-\sigma I_n)^H(A-\sigma I_n)P\hat{y}=\lambda P^H(A-\sigma I_n)^HP\hat{y}.
	\end{align*}		
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs, cont'd\textsubscript{3}}
	...
	\begin{align*}
		G_1:=P^H(A-\sigma I_n)^H(A-\sigma I_n)P
		\text{ and }
		G_2:=P^H(A-\sigma I_n)P
	\end{align*}
	...
	\begin{align*}
		\rho=\frac{y^HAy}{y^Hy}=\frac{\hat{y}^HP^HAP\hat{y}}{\hat{y}P^HPy}
		=\sigma+\frac{\hat{y}^HP^H(A-\sigma I_n)P\hat{y}}{\hat{y}P^HPy}
		=\sigma+\frac{\hat{y}^HG_2\hat{y}}{\hat{y}P^HPy}
	\end{align*}
	so that, if $P^HP=I_m$ and $\hat{y}^H\hat{y}=1$, then we have $\boxed{\rho=\sigma+\hat{y}^HG_2\hat{y}}$.
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs, cont'd\textsubscript{4}}
	...
	\end{align*}
	where, once again, we assume $P^HP=I_m$ and $\hat{y}^H\hat{y}=1$ so that
	\begin{align*}
	\hat{r}^H\hat{r}
...
\begin{frame}{Harmonic Ritz approximation of interior eigenpairs, cont'd\textsubscript{5}}
	...
	Still assuming $P^HP=I_m$ and $\hat{y}^H\hat{y}=1$, we then have
	\begin{align*}
		\tilde{r}^H\tilde{r}
		=&\;(Ay-(\sigma+\lambda)y)^H(Ay-(\sigma+\lambda)y)\\
		=&\;((A-\sigma I_n)y-\lambda y)^H((A-\sigma I_n)y-\lambda y)\\
		=&\;\hat{y}^HP^H(A-\sigma I_n)^H(A-\sigma I_n)P\hat{y}
		   -\lambda\hat{y}^HP^H(A-\sigma I_n)^HP\hat{y}\\
		 &\;-\overline{\lambda}\hat{y}^HP^H(A-\sigma I_n)P\hat{y}+\lambda\overline{\lambda}\\
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors}
	...
	\item Consider the shifted procedure \texttt{Arnoldi}($A-\sigma I_n$, $q_1$, $m$) $\mapsto(Q_{m+1},\underline{H}_m)$ which returns an orthonormal basis $Q_{m}:=[q_1\,\dots\,q_m]$ of $\mathcal{K}_{m}(A-\sigma I_n,q_1)$ such that $\!(A-\sigma I_n)Q_m=Q_{m+1}\underline{H}_m$ where $Q_{m+1}:=[Q_m\,q_{m+1}]$ as well as $\underline{H}_m=Q_{m+1}^H(A-\sigma I_n)Q_m$ and $H_m=Q_m^H(A-\sigma I_n)Q_m$.
	\item Then, harmonic Ritz vectors $y\!\in\!\mathcal{R}(Q_m)$ are such that 
	\begin{align*}
		(A-\sigma I_n)y-\lambda y\perp (A-\sigma I_n)\mathcal{R}(Q_m)
	\end{align*}
	yields the following reduced generalized eigenvalue problem in which we search for non-trivial pairs $(\lambda,\hat{y})\in\mathbb{C}\times\mathbb{C}^{m}$ such that $y=Q_m\hat{y}$ and
	\begin{align*}
		Q_m^H(A-\sigma I_n)^H(A-\sigma I_n)Q_m\hat{y}        =&\;\lambda Q_m^H(A-\sigma I_n)^HQ_m\hat{y}\\
		\underline{H}^H_mQ_{m+1}^HQ_{m+1}\underline{H}_m\hat{y}=&\;\lambda H_m^H\hat{y}\\
		\underline{H}^H_m\underline{H}_m\hat{y}=&\;\lambda H_m^H\hat{y}
	\end{align*}
	\end{itemize}
	\medskip
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{1}}
	...
	\begin{align*}
		(A-\sigma I_n)Q_m=Q_{m}H_m+h_{m+1,m}q_{m+1}e_m^T
	\end{align*}
	allows to rewrite the reduced eigenvalue problem of the harmonic Ritz projection as follows:
	\begin{align*}
		(H_m^HH_m+|h_{m+1,m}|^2e_me_m^T)\hat{y}=\lambda H_m^H\hat{y}
	\end{align*}
	\vspace{-.65cm}
	\begin{align*}
		\hspace{.23cm}\boxed{(H_m+|h_{m+1,m}|^2fe_m^T)\hat{y}=\lambda\hat{y}}
	\end{align*}
	%\vspace{-.45cm}\\
	where $f:=H_m^{-H}e_m\in\mathbb{C}^{m}$ and $e_m:=I_m[:,m]$.
	\item Then, the harmonic eigen-residual $\tilde{r}:=Ay-(\sigma+\lambda )y$ of a given harmonic Ritz approximate eigen-pair $(\sigma+\lambda,y)$ with $y:=Q_m\hat{y}$ is such that
	\vspace{-.1cm}
	\begin{align*}
		\tilde{r}=&\;AQ_m\hat{y}-(\sigma+\lambda)Q_m\hat{y}\\
		=&\;(A-\sigma I_n)Q_m\hat{y}-\lambda Q_m\hat{y}=(Q_{m}H_m+h_{m+1,m}q_{m+1}e_m^T)\hat{y}-\lambda Q_m\hat{y}\\
		=&\;Q_m(\lambda I_n-|h_{m+1,m}|^2fe_m^T)\hat{y}+h_{m+1,m}q_{m+1}e_m^T\hat{y}-\lambda Q_m\hat{y}\\
		=&\;h_{m+1,m}(e_m^T\hat{y})q_{m+1}-|h_{m+1,m}|^2(e_m^T\hat{y})Q_mf
	\end{align*}
	\end{itemize}
\end{frame}
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{2}}
	...
	\begin{align*}
		\tilde{r}
		=&\;h_{m+1,m}(e_m^T\hat{y})Q_{m+1}\begin{bmatrix}0_{m\times 1}\\1\end{bmatrix}-|h_{m+1,m}|^2(e_m^T\hat{y})Q_{m+1}\begin{bmatrix}f\\0\end{bmatrix}\\
		=&\;h_{m+1,m}(e_m^T\hat{y})Q_{m+1}\begin{bmatrix}0_{m\times 1}\\1\end{bmatrix}+h_{m+1,m}(e_m^T\hat{y})Q_{m+1}\begin{bmatrix}-\overline{h_{m+1,m}}f\\0\end{bmatrix}\\
		=&\;h_{m+1,m}(e_m^T\hat{y})Q_{m+1}\begin{bmatrix}-\overline{h_{m+1,m}}f\\1\end{bmatrix}\\
		=&\;\beta_{m}Q_{m+1}s
	\end{align*}
	so that\vspace{-.35cm}
	\begin{align*}
		\boxed{\tilde{r}=\beta_{m}Q_{m+1}s}
		\text{  where  }
		\boxed{\beta_{m}:=h_{m+1,m}e_m^T\hat{y}}
		\text{  and  }
		\boxed{s:=\begin{bmatrix}-\overline{h_{m+1,m}}f\\1\end{bmatrix}}\,.
	\end{align*}
	\item The norm of $\tilde{r}$ is then given by $\boxed{\|\tilde{r}\|_2=|\beta_{m}|(|h_{m+1,m}|^2f^Hf+1)^{1/2}}\,$.
	\end{itemize}
\end{frame}
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{3}}
	\begin{itemize}
		\item When precise eigenvalues are wanted, it is preferred to use the Rayleigh quotient rather than $\sigma+\lambda$. Assuming $\hat{y}$ has unit norm, so does $y:=Q_m\hat{y}$, and the Rayleigh quotient of $y$ is given by
		\begin{align*}
			\rho=\sigma+\hat{y}^HH_m\hat{y}=\sigma+\lambda-|h_{m+1,m}|^2(\hat{y}^Hf)(e_m^T\hat{y}).
		\end{align*}
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{4}}
	\begin{itemize}
	\item As mentioned before, we are interested by the case in which harmonic Ritz approximations are considered in the context of the non-shifted procedure $\mathtt{Arnoldi}(A,q_1,m)\mapsto(Q_{m+1},\underline{H}_m)$ which returns an orthonormal basis $Q_m:=[q_1\dots q_m]$ of $\mathcal{K}_m(A,q_1)$ such that $AV_m=Q_{m+1}\underline{H}_m$ where $Q_{m+1}:=[Q_m\,q_{m+1}]$, $\underline{H}_m=Q_{m+1}^HAQ_m$ and $H_m=Q_m^HAQ_m$.
	\begin{itemize}
	\item Then, the harmonic Ritz vector $y\in\mathcal{R}(Q_m)$ is still such that
	\begin{align*}
		(A-\sigma I_n)y-\lambda y
		\perp
		(A-\sigma I_n)\mathcal{R}(Q_m)
	\end{align*}
	but now yields the following reduced generalized eigenvalue problem:
	\begin{align*}
		((A-\sigma I_n)Q_m)^H(A-\sigma I_n)Q_m\hat{y}=&\;\lambda((A-\sigma I_n)Q_m)^HQ_m\hat{y}\\
		((A-\sigma I_n)Q_m)^HAQ_m\hat{y}-\sigma((A-\sigma I_n)Q_m)^HQ_m\hat{y}=&\;\lambda((A-\sigma I_n)Q_m)^HQ_m\hat{y}
	\end{align*}
	so that
	\begin{align*}
	((A-\sigma I_n)Q_m)^HAQ_m\hat{y}=&\;(\sigma+\lambda)((A-\sigma I_n)Q_m)^HQ_m\hat{y}
	\end{align*}
	\end{itemize}
	\end{itemize}
\end{frame}
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{5}}
	...
	\begin{align*}
		((AQ_m)^HAQ_m-\overline{\sigma}Q_m^HAQ_m)\hat{y}=&\;(\sigma+\lambda)(Q_m^HA^HQ_m-\overline{\sigma}I_m)\hat{y}\\
		(H_m^HH_m+|h_{m+1,m}|^2e_me_m^T-\overline{\sigma}H_m)\hat{y}=&\;(\sigma+\lambda)(H^H_m-\overline{\sigma}I_m)\hat{y}\\
		((H_m^H-\overline{\sigma}I_m)H_m+|h_{m+1,m}|^2e_me_m^T)\hat{y}=&\;(\sigma+\lambda)(H_m-\sigma I_m)^H\hat{y}\\
		((H_m-\sigma I_m)^HH_m+|h_{m+1,m}|^2e_me_m^T)\hat{y}=&\;(\sigma+\lambda)(H_m-\sigma I_m)^H\hat{y}
	\end{align*}
	to finally yield
	\begin{align*}
		\boxed{(H_m+|h_{m+1,m}|^2fe_m^T)\hat{y}=(\sigma+\lambda)\hat{y}}
		\text{ where }
		\boxed{f:=(H_m-\sigma I_m)^{-H}e_m},
	\end{align*}
...
\begin{frame}{Arnoldi procedure with harmonic Ritz vectors, cont'd\textsubscript{6}}
	...
	\begin{align*}
		\rho=y^HAy=\hat{y}^HV_m^HAV_m\hat{y}=\hat{y}^HH_m\hat{y}=\sigma+\lambda-|h_{m+1,m}|^2(\hat{y}^Hf)(e_m^T\hat{y}).
	\end{align*}
	...
	\begin{align*}
		\tilde{r}
		=&\;AQ_m\hat{y}-(\sigma+\lambda)Q_m\hat{y}\\
		%=(Q_mH_m+h_{m+1,m}q_{m+1}e_m^T)\hat{y}-(\sigma+\lambda)Q_m\hat{y}\\
		=&\;Q_mH_m\hat{y}+h_{m+1,m}q_{m+1}e_m^T\hat{y}-(\sigma+\lambda)Q_m\hat{y}\\
		=&\;(\sigma+\lambda)Q_m\hat{y}-|h_{m+1,m}|^2Q_mfe_m^T\hat{y}+h_{m+1,m}q_{m+1}e_m^T\hat{y}-(\sigma+\lambda)Q_m\hat{y}\\
		=&\;-|h_{m+1,m}|^2(e_m^T\hat{y})Q_mf+h_{m+1,m}q_{m+1}e_m^T\hat{y}
	\end{align*}
	which, similarly as before, can be recast into\vspace{-.2cm}
	\begin{align*}
		\tilde{r}=\beta_mQ_{m+1}s
		\;\text{ where }\;
		\beta_m:=h_{m+1,m}e_m^T\hat{y}
		\;\text{ and }\;
		s=\begin{bmatrix}-\overline{h_{m+1,m}}f\\1\end{bmatrix},
	\end{align*}
...
\item When $A$ is symmetric (i.e., $A = A^T$), the Hessenberg matrix $H_m = Q_m^T A Q_m$ is symmetric too.
...
\begin{itemize}\normalsize
\item[-] An orthonormal basis $Q_m = [q_1, q_2, \ldots, q_m]$ for the Krylov subspace $\mathcal{K}_m(A, q_1)$
\item[-] A tridiagonal matrix $T_m = Q_m^T A Q_m$ with diagonal elements $\alpha_i$ and off-diagonal elements $\beta_i$
\end{itemize}
\end{itemize}
\end{frame}
...
\item A \textbf{Rayleigh-Ritz vector} $y\in\mathcal{R}(Q_m)$ approximates an eigenvector of $A$ with the \textbf{Ritz value} $\lambda$ such that $Ay-\lambda y\perp \mathcal{R}(Q_m)$. That is, we \textbf{search for} $(\lambda,\hat{y})\in\mathbb{C}\times\mathbb{C}^{m}\setminus\{0\}$ s.t. $z^H\left(Ay-\lambda y\right)=0\;\forall\; z\in\mathcal{R}(Q_m)$ with $\!y=Q_m\hat{y}$.
This \textbf{simplifies to}
\begin{align*}
Q_m^H\left(AQ_m\hat{y}-\lambda Q_m\hat{y}\right)=&\;0\\
T_m\hat{y}-\lambda\hat{y}=&\;0
\;\;\implies\;\;
\boxed{T_m\hat{y}=\lambda\hat{y}}
\end{align*}
where use is made of the Lanczos relation and $Q_m^HQ_m=I_n$.
\item The eigen-residual $\tilde{r}:=Ay-\lambda y$ can be computed as:
\begin{align*}
\tilde{r} = AQ_m \hat{y} - \lambda Q_m \hat{y} = \beta_m (e_m^T \hat{y}) q_{m+1}
\end{align*}
\item This means $\|\tilde{r}\|_2 = \beta_m \|e_m^T \hat{y}\|$, providing a simple way to assess convergence without explicitly computing $Ay$.
\end{itemize}
\end{frame}
...
\item A \textbf{harmonic Ritz vector} $y\in\mathcal{R}(Q_m)$ approximates an eigenvector of $A$ with the \textbf{harmonic Ritz value} $\lambda$ such that 
\begin{align*}
(A-\sigma I_n)y-\lambda y\perp (A-\sigma I_n)\mathcal{R}(Q_m).
\end{align*}
That is, we \textbf{search for} $(\lambda,\hat{y})\in\mathbb{C}\times\mathbb{C}^{m}\setminus\{0\}$ s.t.
\begin{align*}
Q_m^H(A-\sigma I_n)^H(A-\sigma I_n)Q_m\hat{y}        =&\;\lambda Q_m^H(A-\sigma I_n)^HQ_m\hat{y}\\
\underline{T}^T_m\underline{T}_m\hat{y}=&\;\lambda T_m^H\hat{y}.
\end{align*}
Using the Lanczos relation, this is recast into
\begin{align*}
(T_m+|\beta_m|^2fe_m^T)\hat{y}=\lambda\hat{y}
\end{align*}
$f:=T_m^{-H}e_m\in\mathbb{C}^{m}$ and $e_m:=I_m[:,m]$.
\end{itemize}
\end{frame}
...
\begin{itemize}\normalsize
\item[-] \textbf{Arnoldi process:} For general matrices, produces a Hessenberg matrix $H_m$, requires orthogonalization against all previously formed vectors
\item[-] \textbf{Lanczos process:} For symmetric matrices, produces a tridiagonal matrix $T_m$, relies on a three terms recurence formula
\end{itemize}
\item Both methods:
\begin{itemize}\normalsize
\item[-] Construct an orthonormal basis for the Krylov subspace $\mathcal{K}_m(A,v)$
...
\end{bmatrix},
\end{align*}
(a) Find the Rayleigh Ritz pairs of $A$ with respect to $\mathcal{R}(V)$.\vspace{.1cm}\\
(b) Assemble the reduced eigenvalue problem to solve in order to find the harmonic Ritz values of $A$ with respect to $\mathcal{R}(V)$ for $\sigma=0$.
