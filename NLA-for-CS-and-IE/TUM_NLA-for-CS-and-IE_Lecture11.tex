...
\end{center}
\item As it turns out, the eigenpairs of the leading block $H_k$ of $H$ are good approximations of some eigen-pairs of $A$.
\end{itemize}
\end{frame}
...
\item Then, the harmonic eigen-residual $\tilde{r}:=Ay-(\sigma+\lambda )y$ of a given harmonic Ritz approximate eigen-pair $(\sigma+\lambda,y)$ with $y:=Q_k\hat{y}$ is such that
\vspace{-.1cm}
\begin{align*}
...
Then, the \textbf{search for} for the pair $(\sigma+\lambda,\hat{y})\in\mathbb{R}\times\mathbb{R}^{k}\setminus\{0\}$ is such that
\begin{align*}
...
G_1\hat{y}=&\;(\sigma+\lambda)G_2\hat{y}
\end{align*}
where $G_1:=T_k(T_k-\sigma I_k)+\beta_k^2e_ke_k^T$ and $G_2:=T_k-\sigma I_k$.
\item[] \textbf{{\color{red}{Problem}}}: Even if $A$, and thus $T_k$ are positive definite, $G_2$ is likely \textbf{not positive definite}, which means the \textbf{harmonic Ritz pairs} are \textbf{{\color{red}{complex}}}.
\item[] \underline{Question}: \textbf{How do you build real eigenpairs by harmonic Ritz?}
\end{itemize}
\end{frame}
