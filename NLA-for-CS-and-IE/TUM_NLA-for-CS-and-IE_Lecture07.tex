...
\item Building a QR factorization costs $\mathcal{O}(mn^2)$ FLOPs.
For large sparse matrices, this can be an overkill.
...
\item[] Let $U_k:=[u_1,\dots,u_k]$, $V_k:=[v_1,\dots,v_k]$ and
\begin{align*}
B_k:=
\begin{bmatrix}
\alpha_1&&&\\
...
\end{align*}
\end{itemize}
\tiny{Golub, G., \& Kahan, W. (1965). Calculating the singular values and pseudo-inverse of a matrix. Journal of the Society for Industrial and Applied Mathematics, Series B: Numerical Analysis, 2(2), 205-224.}
\end{frame}
...
r_k\in&\,\mathcal{K}_k(AA^T,u_1).
\end{align*}
Equivalently, that is, we have $x_k=x_0+V_ky_k$ such that
\begin{align*}
\hspace{-.2cm}
x_k=\underset{x\in x_0+\text{range}(V_k)}{\arg\min}\|b-Ax\|_2
\implies
y_k=&\,\underset{y\in\mathbb{R}^k}{\arg\min}\|b-A(x_0+V_ky)\|_2\\
=&\,\underset{y\in\mathbb{R}^k}{\arg\min}\|r_0-U_{k+1}B_ky\|_2\\
=&\,\underset{y\in\mathbb{R}^k}{\arg\min}\|U_{k+1}(\beta_1e_1)-U_{k+1}B_ky\|_2\\
=&\,\underset{y\in\mathbb{R}^k}{\arg\min}\|\beta_1e_1-B_ky\|_2.
\end{align*}
...
\begin{itemize}
\item Puting it out all together, this leads to the following algorithm:
\begin{itemize}
...
\item[7.]\hspace{.2cm}$y_i:=\underset{y\in\mathbb{R}^i}{\arg\min}\|\beta_1e_1-B_iy\|_2$\vspace{.1cm}
\end{itemize}
When convergence is achieved, the iterate $x_i:=x_0+V_iy_i$ is formed.
\item[]Convergence monitoring is reliant on the evaluation of $\|r_i\|_2$.
Note that we have
\begin{align*}
\|r_i\|_2
=\|b-A(x_0+V_iy_i)\|_2
=\|\beta_1e_1-B_iy_i\|_2
\end{align*}
so that $x_i$ needs not be assembled to evaluate the residual norm.
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item Implement and test your implementation of Householder QR.
