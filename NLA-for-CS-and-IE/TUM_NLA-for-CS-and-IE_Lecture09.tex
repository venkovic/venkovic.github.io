...
\begin{itemize}
\item \textbf{Computing eigenvalues and -vectors is} a very \textbf{difficult} task.\vspace{.1cm}\\
There is \textbf{no direct method} for computing eigenvalues of matrices of size five or higher in general.\vspace{.1cm}\\
That is, there is \textbf{no algorithm} that \textbf{can compute eigenvalues exactly} assuming perfect arithmetic.
...
\end{center}
such that, if we pick $u=[1\,z\,z^2\,\dots\,z^{n-1}]^T$ where $z$ is a root of $p(x)$, then we have $Au=zu$ so that $(z,u)$ is an eigen-pair of $A$.\vspace{.1cm}\\
...
\end{center}
One thing about the eigen-decomposition is that powers of $A$ are such that
\begin{align*}
...
\begin{itemize}
\item The power iteration is based on this idea of taking powers of $A$ to approximate the largest eigen-pair.
The algorithm is as follows:\vspace{.2cm}
\begin{itemize}
...
where $(\lambda^{(k)},q^{(k)})$ is an iterate approximating the largest eigen-pair of $A$.\vspace{.05cm}\\
At the $k$-th step, the approximate eigenvector is 
\begin{align*}
q^{(k)}=A^kq^{(0)}/\|A^kq^{(0)}\|_2,
\end{align*}
and the corresponding approximate eigenvalue is $\lambda^{(k)}=q^{(k)}{}^HAq^{(k)}$.\vspace{.05cm}\\
Note that, even though $q^{(k)}$ is formed with $A^k$, the matrix power $A^k$ is not explicitly computed.\vspace{.05cm}\\
...
\begin{itemize}
\item Let us assume again that the eigen-pairs $(\lambda_1,x_1),\dots,(\lambda_n,x_n)$ of $A$ are ordered such that $|\lambda_1|>|\lambda_2|\geq\dots\geq|\lambda_n|$.
...
\begin{itemize}
\item Assume we are equiped with an approximation $\mu$ of the eigenvalue $\lambda_i$ of $A$.
\item An \textbf{inverse $\!$iteration} uses $\mu$ to $\!$form an abritrarily good $\!$approximation of $\lambda_i.\hspace{-1cm}$
\item If $\mu$ is a good approximation of $\lambda_i$, then\vspace{.1cm}\\
The shifted matrix $A-\mu I_n$ has a small eigenvalue $\lambda_i-\mu$.\vspace{.1cm}\\
The shift-and-invert matrix $(A-\mu I_n)^{-1}$ has a large eigenvalue $1/(\lambda_i-\mu)$.\vspace{.1cm}\\
So, a power iteration applied to $(A-\mu I_n)^{-1}$ should allow us to calculate $x_i$ very quickly, since $1/(\lambda_i-\mu)$ is now the largest eigenvalue, with the corresponding eigenvector $x_i$.\vspace{.1cm}\\
The algorithm of inverse iteration is as follows:\vspace{.1cm}
\begin{itemize}
...
\item[4.]\hspace{.4cm}Solve for $z^{(k)}$ such that $(A-\mu I_n)z^{(k)}=q^{(k)}
\color{gray}{\;//\;z^{(k)}:=(A-\mu I_n)^{-1}q^{(k)}}$\vspace{.07cm}
...
\item Let us assume the matrix $A$ is real and symmetric so that its eigenvalues and -vectors are real, and the eigenvectors are orthogonal.
...
More often than not, the zeros of $\nabla r$ are saddle points, as the Rayleigh quotient is only minimized (resp. maximized) at the smallest eigen-pair (resp. largest eigen-pair).\vspace{.1cm}\\
...
However, in general, iterative methods for computing a single eigen-pair have limited applicability.
...
\item[5.]\hspace{.4cm}$q_1:=Aq_1;\;q_2:=Aq_2$\vspace{.07cm}
\item[6.]\hspace{.4cm}Project $q_2$ onto the space orthogonal to $q_1$ {\color{gray}// E.g., $q_2:=\left(I_n-\frac{q_1q_1^T}{q_1^Tq_1}\right)q_2$}\vspace{.07cm}
\item[7.]\hspace{.4cm}$q_1:=q_1/\|q_1\|_2,\;q_2:=q_2/\|q_2\|_2$\vspace{.07cm}
\item[8.] Return $q_1^TAq_1$ and $q_2^TAq_2$\vspace{.2cm}
\end{itemize}
...
\begin{align*}
q_2^TAq_1\approx q_2Ax_1=\lambda_1q_2^Tx_1\approx\lambda_1q_2^Tq_1=0
\end{align*}
...
\end{align*}
where $R_{k+1}$ is upper triangular, and $U_{k+1}$ is orthogonal.\vspace{.1cm}\\
Note that $U_{k+1}R_{k+1}$ is a QR decomposition of $T_k=Q_k^TAQ_k$.\vspace{.1cm}\\
This yields the following pseudocode to compute the eigenvalues of $A$ :\vspace{.2cm}
\begin{itemize}
...
\item QR iteration:
\begin{itemize}
\item[-] Implementation sometimes requires tridiagonalization,\\
available for general matrices\vspace{.07cm}
\item[-] Fastest to compute the eigendecomposition of small matrices, i.e., for $n\leq 25$\vspace{.07cm}
\item[-] Algorithm behind the Matlab, NumPy and Julia functions \texttt{eig}\vspace{.07cm}
...
\begin{itemize}
\item[-] Implementation requires tridiagonalization, available for symmetric matrices\vspace{.07cm}
